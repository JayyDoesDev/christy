...
#
# This Cakefile currently looks for src so beware! 
# It comes baked in with 5 tasks:
#
# * build - compiles your src directory to your water directory
# * watch - watches any changes in your src directory and automatically compiles to the water directory
...
# **and** defaulted options to compile src to water
...
unlinkIfCoffeeFile = (file) ->
  if file.match /\.coffee$/
    fs.unlink file.replace('src','js').replace(/\.coffee$/, '.map'), ->
    fs.unlink file.replace('src','js').replace(/\.coffee$/, '.js'), ->
    true
  else false
